...
const startServer = (app: Application): void => {
  try {
    ...
    log.info(`Gig Server with process id ${process.pid} has started!`);
    httpServer.listen(SERVER_PORT, () => {
      log.info(`Gig Server is listening on port ${SERVER_PORT}`);
    });
  } catch (error) {
    log.log('error', 'Gig Service startServer() method:', error);
  }
};
